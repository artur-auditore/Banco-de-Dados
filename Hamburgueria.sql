...
SELECT REALIZAR_PEDIDO(2, 'Ragnar Lothbrok', 'Hector', 'AVALON ZONA LESTE', 1, 'Marta', 'Local');
SELECT REALIZAR_PEDIDO(3, 'The Hobbit', 'Hector', 'AVALON ZONA LESTE', 1, 'Daniel', 'Local');
...
CREATE OR REPLACE FUNCTION MODIFICA_STATUS(STATUS TEXT, COD_PED INT)
RETURNS VOID AS $$
BEGIN
	IF (NOT EXISTS(SELECT * FROM PEDIDO WHERE COD_PEDIDO=$2)) THEN
	  RAISE 'Pedido não existe';
	ELSIF(NEW.STATUS ILIKE 'CANCELADO' OR NEW.STATUS ILIKE 'FINALIZADO' OR NEW.STATUS ILIKE 'CANCELADO') THEN
	ELSE
		RAISE 'Status inválido!';
	END IF;
...
SELECT MODIFICA_STATUS('CANCELADO', 1);
SELECT * FROM PEDIDO;
...
BEGIN
	...
	SELECT MODIFICA_STATUS('CANCELADO', $1) INTO retorno;
END;
...
BEGIN
	IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
		...
		ELSIF (TG_TABLE_NAME = 'PEDIDO' OR TG_TABLE_NAME = 'pedido') THEN
			IF (NEW.COD_PEDIDO IS NULL OR NEW.COD_FUNC IS NULL OR NEW.COD_CLIENTE IS NULL OR NEW.STATUS IS NULL OR NEW.DATA_PEDIDO IS NULL OR NEWVALOR_TOTAL IS NULL OR NEW.ENTREGA) THEN
				RAISE EXCEPTION 'Há valores nulos. Tente novamente.';
			END IF;
			
		ELSIF (TG_TABLE_NAME = 'ITEM_PEDIDO' OR TG_TABLE_NAME = 'item_pedido') THEN
			IF (NEW.COD_ITEM_PEDIDO IS NULL OR NEW.COD_PED IS NULL OR NEW.COD_REST IS NULL OR NEW.COD_PRATO IS NULL OR NEW.PRECO IS NULL OR NEW.QUANTIDADE IS NULL) THEN
				RAISE EXCEPTION 'Há valores nulos. Tente novamente.';
			END IF;
		END IF;
	END IF;
...
CREATE TRIGGER CHECK_NULO BEFORE INSERT OR UPDATE ON PEDIDO
FOR EACH ROW EXECUTE PROCEDURE CHECK_NULO();

DROP TRIGGER IF EXISTS CHECK_NULO ON PEDIDO;

CREATE TRIGGER CHECK_NULO BEFORE INSERT OR UPDATE ON ITEM_PEDIDO
FOR EACH ROW EXECUTE PROCEDURE CHECK_NULO();
...
DECLARE
	QUANT INT;	
BEGIN
	SELECT QUANTIDADE INTO QUANT FROM ESTOQUE WHERE COD_REST = NEW.COD_REST  AND COD_PRATO = NEW.COD_PRATO;
	IF (QUANT IS NOT NULL) THEN
		IF (QUANT < NEW.QUANTIDADE) THEN		  
		  RAISE 'Estoque insuficiente!'; 
		END IF;
	ELSE
		RAISE 'Não existe este produto no estoque';
	END IF;
	RETURN NEW;
END;
$$ LANGUAGE plpgsql

CREATE TRIGGER CHECAR_ESTOQUE BEFORE INSERT OR UPDATE ON ESTOQUE
FOR EACH ROW EXECUTE PROCEDURE CHECAR_ESTOQUE_DISPONIVEL();

DROP TRIGGER IF EXISTS CHECAR_ESTOQUE ON ESTOQUE;

...
BEGIN
	...
	SELECT COD_REST INTO restaurant FROM RESTAURANTE WHERE RAZAO_SOCIAL = $4;
	SELECT COD_CLIENTE INTO client FROM CLIENTE WHERE nome = $6;
	...
	IF (restaurant IS NOT NULL AND client IS NOT NULL AND prato IS NOT NULL) THEN
		IF (EXISTS(SELECT * FROM PEDIDO WHERE COD_PEDIDO=$1)) THEN
			...
			INSERT INTO ITEM_PEDIDO VALUES(DEFAULT, $1, prato, restaurant, valor, $5);
			UPDATE ESTOQUE SET QUANTIDADE  = (SELECT QUANTIDADE FROM ESTOQUE WHERE COD_RESTAURANTE = restaurant AND COD_PRATO = prato) - $5
			WHERE COD_RESTAURANTE = restaurant AND COD_PRATO = prato;
...
SELECT REALIZAR_PEDIDO(1, 'Drogon', 'Leticia', 'AVALON', 1, 'Artur', 'Zona sul');
